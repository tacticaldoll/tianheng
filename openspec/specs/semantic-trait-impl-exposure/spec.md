...
`(target, rule, finding)` baseline identity (the one forbidden false negative). Findings SHALL share
...
- **THEN** the `self` finding still reacts: its seam names its own position, so the baseline identity `(target, rule, finding)` does not mask it
