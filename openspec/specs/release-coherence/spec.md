...
- **THEN** the repository is checked as **development**, not as a snapshot with a modified tree. The state is
  ...
  the `0.6.0` window: its first change could not pass the Definition of Done until it was committed, and passed
...
section MAY carry a migration for a break marked some other way.
...
- **THEN** nothing reacts, and the leak is real: an adopter reading a released section can meet entries naming
  files they can never run. What is refused is the **repair**, not the diagnosis — rewriting a dated section
  to satisfy a rule written afterwards would falsify the record rather than repair it — so this is a declared
  false negative with an owner rather than a shape that is harmless. Closing it
  needs a repair that adds to the record instead of editing it.
- **AND** the instance that settled this bound's classification is **unreachable rather than absent**: it sat
  in a released section this repository no longer carries. The bound is reached again by the first released
  section it does carry that names a path since deleted.
