...
Every typed declaration SHALL carry exactly one `Defence`: either non-empty `PinnedBy { first, additional }`
evidence or `Unpinned { tracker }`.
The two states SHALL be mutually exclusive in the type, matching the register's `PINNED-BY` / `UNPINNED`
grammar. Multiple `PINNED-BY` lines on one scenario SHALL all be retained in declaration order. An unpinned
declaration SHALL carry a tracker and no fabricated test name.
...
- **THEN** the typed declaration retains every test while its pinned state remains non-empty by construction
