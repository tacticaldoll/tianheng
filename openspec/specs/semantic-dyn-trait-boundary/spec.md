...
- **THEN** the second still reacts: its canonical fact descriptor and therefore `finding_key` differ from the first. The dimension MUST preserve every observable distinguishing payload in that descriptor (never collapse the realistic shapes above to one key — which would silently pass a new exposure under a baselined one, the one forbidden bug); its human rendering remains diagnostic rather than identity
...
- **THEN** the second still reacts: the fact descriptor is **seam-qualified** (`{rendered shape} exposed by {seam}`, the seam being the owning item / sub-element as in `semantic-signature-coupling`), so two seams sharing a shape do not collapse to one `(target, rule, finding_key)` and baselining one MUST NOT mask the other (the one forbidden bug)
...
- **THEN** the system does not claim to distinguish them: they share a canonical fact descriptor and key (each still *reacts* on first occurrence; only baseline-dedup granularity is bounded). This is a **stated descriptor bound** — the same `(target, rule, finding_key)` granularity bound `semantic-trait-impl-locality`'s `(impl for <self_ty>)` fact carries — declared here, never a silent claim of cleanliness
...
shared violation identity `(target, rule, finding_key)` (the finding **seam-qualified** as
