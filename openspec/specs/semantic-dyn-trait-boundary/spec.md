...
shared violation identity `(target, rule, finding)`; and the violation report identifies the
governed anchor, the rule (`must not expose dyn`), the offending finding (the exposed
trait-object, named where resolvable), and the boundary's reason. The AST observation SHALL
