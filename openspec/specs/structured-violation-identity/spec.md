...
family it owns and every finite, typed nested discriminator vocabulary that changes a fact code,
field set, or canonical field value. The reaction SHALL pin the version-2 namespace, fact code,
canonical field names, and representative canonical field values. Each dimension SHALL additionally
inspect at least one violation produced through its real boundary reaction to pin target and rule as
outer identity roles rather than values folded into the finding key.
Adding a fact variant or finite identity-bearing discriminator variant SHALL require an explicit
catalog decision rather than silently bypassing the reaction. The reaction SHALL NOT freeze human
finding text, complete report JSON, or diagnostic metadata. A helper whose rendered output enters a
key SHALL be documented and tested as wire-sensitive even when it also serves readability.
...
- **THEN** every fact variant and finite typed identity-bearing nested discriminator owned by each dimension has an exact expected namespace, code, named fields, and representative values

#### Scenario: A new fact family cannot bypass the catalog

- **WHEN** a dimension adds a new fact or finite identity-bearing discriminator variant without classifying its identity schema
