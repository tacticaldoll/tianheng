...
#### Scenario: A reason spanning more than one line is projected in full

- **WHEN** a boundary's declared reason contains a newline
- **THEN** the Markdown projection carries every line of it, attributable to that boundary — a reason is
  free text and `because` restricts nothing, so a projection that marks only the first line renders the rest
  as body text and carries less than the constitution holds
