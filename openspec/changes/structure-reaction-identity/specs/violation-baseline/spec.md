...
**Reason**: 0.3.0 intentionally removes the text-identity adapter and numeric baseline generations;
retaining an upgrade path would keep presentation identity inside the pure reaction model.

**Migration**: Preserve any desired `owner`/`tracker` annotations externally, move or delete the
unsupported baseline, run `tianheng check --write-baseline <file>`, then restore annotations onto
the newly observed matching facts.
