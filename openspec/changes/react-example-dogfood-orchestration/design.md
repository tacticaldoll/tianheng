...
- React when a non-comment line in the positive driver directly names a focused matrix basename.
...
Define non-recursion at the observable source level: after dropping full-line shell comments, no remaining driver line may contain a focused matrix basename. This catches direct invocations including workspace-prefixed paths without claiming to resolve dynamically assembled command names.

## Risks / Trade-offs

- [The source check can reject a basename in an executable diagnostic string] → This is the deliberately narrow authored-form contract; keep those names out of driver code entirely and place explanatory references in comments.
