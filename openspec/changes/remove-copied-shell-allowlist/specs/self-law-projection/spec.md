...
Authored Rust source under the `tianheng` shell SHALL NOT restate the shell dependency declaration with
`restrict_dependencies_to(`. A comment that explains the dependency-free implementation SHALL point to
the generated self-law projection instead of copying the declaration or its current members.
...
#### Scenario: Authored shell source does not copy the dependency declaration

- **WHEN** the repository self-governance tests enumerate Rust source under `crates/tianheng/src`
- **THEN** no source file contains `restrict_dependencies_to(`, so dependency membership remains owned by the constitution and its generated projection
