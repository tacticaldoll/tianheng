...
- [ ] 3a.1 Widen the restatement reaction: every declared `restrict_dependencies_to` allowlist, read against
  every tracked governance document, not the shell's allowlist against one crate's line comments.
- [ ] 3a.2 See it red first against the instance already in the tree: `PROJECT.md` names
  `serde_json, xuanji, xingbiao` — every member of `guibiao`'s live allowlist.
- [ ] 3a.3 Retire that census in favour of a pointer to `AGENTS.self-law.md`, keeping the prose that a
  declaration cannot carry: why the boundary exists and what it protects.
- [ ] 3a.4 Sweep the remaining allowlists against every governance document and repair what the widened
