...
- **WHEN** the forbidden operand is written as a re-exported facade path (e.g. `crate::Port`, a `pub use` of `crate::ports::Port`) and the module exposes `dyn crate::ports::Port`
- **THEN** the system emits a violation, because both canonicalize through the re-export closure to the same defining path — closing the re-export false negative
