...
- [ ] 1.1 In `crates/kanhe/tests/merge_workflow.rs`, teach the controlled `gh` to **resolve** the body it is
...
- [ ] 1.2 Make the controlled `gh` log its arguments **newline-safely**, so one invocation occupies one log
...
- [ ] 1.3 Add a controlled-`cargo` mode that rewrites the body file while standing where the gate runs, so the
      interval between the judgement and the merge is reproducible rather than argued about.
- [ ] 1.4 Run the whole of `merge_workflow.rs` with the harness changed and the wrapper **unchanged**; every
...
- [ ] 2.1 Add the direction for the delta spec's scenario *The body file changes between the gate and the
...
- [ ] 2.2 Run that direction against the **unchanged** wrapper and record the observed failure verbatim. This
      is the negative run the pull request's `## Verification` must carry; a guard is not a guard until it has
      been seen to fail.

## 3. The repair

- [ ] 3.1 In `scripts/merge-pr.sh`, replace `--body-file "$body_file"` with `--body "$body"` on the final
      `exec`. Nothing else moves: the read stays once, guarded, before the gate, with its cannot-judge refusal
      intact.
- [ ] 3.2 Record in the pull request that `gh pr merge` accepts `-b, --body text` at **gh 2.95.0** — the same
...
- [ ] 3.3 Confirm the allowlist still refuses a caller's `--body`, `--body=*`, `--body-file`, `--body-file=*`,
      ...
      argument order.
- [ ] 3.4 Sweep the tree for any other direction or document asserting the wrapper passes `--body-file`, and
      repair each — the retired spelling must not survive anywhere a reader would take it as current.

## 4. Verify the repair

- [ ] 4.1 The new direction passes, and the whole of `merge_workflow.rs`, `merge_message.rs`,
      `gate_exit_classes.rs` and `gate_identity.rs` passes.
- [ ] 4.2 Confirm the wrapper's exit classes are untouched: `2` for a misconfigured invocation or an input it
      could not read, `1` only for a gate that ran and refused.

## 5. Record

- [ ] 5.1 Add the `CHANGELOG.md` entry under `### Self-governance` in `[Unreleased]`. State the guarantee and
...
- [ ] 5.3 Run `TIANHENG_WORKSPACE_TESTS=1 cargo test -p kanhe --test release_coherence` and confirm the
      changelog placement is clean.

## 6. Definition of Done and lifecycle

- [ ] 6.1 Run the full Definition of Done block from `AGENTS.md` from the workspace root, including the
