...
- [ ] Wire `validate_path_operands` into `trait_impl.rs::trait_impl_findings` (over `allowed`,
...
- [ ] Add regression tests in `crates/hunyi/src/tests.rs`: a malformed-`allowed_locations`
      ...
      <!-- id: 5 -->
- [ ] Close the `BACKLOG.md` `ACCEPTED DEBT` entry for this gap (move to `BUILT / HISTORY`), having
      confirmed both named call sites are fixed, and add the matching `CHANGELOG.md` `[Unreleased]`
      → `### Fixed` entry. <!-- id: 6 -->
- [ ] Full-file grep `openspec/specs/semantic-trait-impl-locality/spec.md` and
