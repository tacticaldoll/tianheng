...
- [ ] 1.1 Narrow `is_self_dependency` in `crates/guibiao/src/cargo_metadata.rs` to also require
...
- [ ] 2.1 Add a unit-level fixture test in `crates/guibiao/src/tests.rs` (alongside the existing
...
- [ ] 2.2 Add a real-entry-point integration test under `crates/guibiao/tests/` that builds a
...
- [ ] 2.3 Re-run the existing self-path-dependency exemption tests unmodified and confirm they
      still pass (the legitimate exemption must survive the narrowing).

## 3. Verification

- [ ] 3.1 Run the full Definition of Done command list from `AGENTS.md` and confirm every command
      passes.
