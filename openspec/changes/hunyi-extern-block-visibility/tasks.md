...
- [ ] Reproduce the gap directly against the real public API (`hunyi::check_visibility`) with a
...
- [ ] Grep for every existing caller of `item_observation`/`item_observation_parts` in
      `crates/hunyi/src` before widening their signature, confirming `visibility_findings` is the
      only consumer. <!-- id: 1 -->
- [ ] Widen `item_observation_parts`/`item_observation` (`crates/hunyi/src/syn_util.rs`) from
...
- [ ] Add regression coverage in `crates/hunyi/src/tests.rs`: multiple `pub` foreign items in one
      ...
      ceiling. <!-- id: 4 -->
- [ ] Verify non-vacuously: revert the `syn_util.rs`/`visibility.rs` fix, confirm every new
...
- [ ] Add the adopter-facing `CHANGELOG.md` `[Unreleased]` → `### Fixed` entry beside `ed19dce`'s
...
- [ ] Modify `semantic-visibility-boundary`'s "Bare-pub item observation" requirement (delta under
...
- [ ] Run the full Definition of Done from the workspace root (build, four clippy passes including
