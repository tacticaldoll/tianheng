...
reference. `law_restatement` excludes it alongside `CHANGELOG.md`.
...
commit message or a dated changelog section — is a measurement of the moment it was taken and
...
narrower than their claims. The record carriers stand outside it by construction, and they are the same two
this file already names — a commit message and a dated `CHANGELOG.md` section: each is a
...
convention. **What a published version records is asked of the tarball and the tag, never of a copy kept
here.** A sha1 exists only after the upload that records it and the registry then holds it permanently, so a
committed copy is late by construction and is the half that can go stale.
