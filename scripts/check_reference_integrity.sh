    ...
    *'*'*) continue ;;
    ...
    tests/*)
      ...
      for member in $members; do
        if [ -e "crates/$member/$reference" ]; then
      ...
      if [ -z "$target" ]; then
        offenses=$((offenses + 1))
        echo "$file: references '$reference', which exists under no workspace member"
      fi
      continue
      ;;
    esac
    ...
    if [ ! -e "$target" ]; then
      offenses=$((offenses + 1))
      echo "$file: references '$reference', which does not exist"
    fi
