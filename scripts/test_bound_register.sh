...
# The census scan distinguishes an unreadable repository from grep's ordinary no-match status. Move the
# fixture immediately after the final tracked-Markdown enumeration: every earlier observation succeeds, then
# the directory transition itself fails at the exact boundary this guard owns.
...
[[ $census_status -eq 2 && $census_output == *'could not enter the repository while scanning tracked Markdown'* ]] \
    || { printf 'an unreadable census root must exit 2 distinctly, got %d: %s\n' "$census_status" "$census_output" >&2; exit 1; }
