...
/// [`crate::module_resolve`]'s single-module-anchored descent does **not** follow (cfg-conditional
/// → following it cfg-blind could read a file rustc does not compile here) and must therefore skip
/// rather than govern the wrong conventional file — a stated, fail-loud bound narrower than the
/// whole-crate walks' own policy (`crates/hunyi/src/scan.rs`'s `resolve_child_modules` UNIONS the
/// conventional file and the `cfg_attr` target instead of skipping; this predicate's only remaining
/// caller is the targeted resolver). The **unconditional** `#[path = "…"]` form is instead *followed*
/// via [`direct_path_value`]; this predicate still reports `true` for it, but callers consult
/// `direct_path_value` first. Recognizes **both** the direct
/// `#[path = "…"]` and the combined `#[cfg_attr(<pred>, …, path = "…")]` spelling (equivalent to
/// `#[cfg(<pred>)] #[path = "…"]`), including arbitrarily **nested** `cfg_attr`
/// (`#[cfg_attr(a, cfg_attr(b, path = "…"))]`). It matches only a genuine `path = "…"`
/// **name-value** meta (the only valid `#[path]` form), so a `#[cfg_attr(<pred>, deprecated)]` on a
/// normal file module is **not** mistaken for a remap (which would drop a governed module — the
...
/// or `None`. This is the value the whole-crate walks and the targeted resolver *follow* to
...
/// here: [`crate::scan`]'s whole-crate walks extract it separately via
/// [`cfg_attr_path_value`] and union it with the conventional file, while
/// [`crate::module_resolve`]'s targeted resolver still treats any `cfg_attr`-wrapped `path` as a
/// skip bound via [`has_path_attr`]. A module has at most one applied unconditional `#[path]`, so
/// the first match is the value.
