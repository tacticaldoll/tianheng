...
/// `check_all` — for a non-empty bundle, the latter reads `metadata` **once** and evaluates all
/// eight capabilities into one accumulator, so the single-read and error-supersedes semantics are
