...
fn the_register_includes_itself() {
    ...
    assert_eq!(
        ...
        Some("crates/jiaochou/tests/projection_register.rs"),
...
fn render(documents: &BTreeMap<String, Registered>, holders: &[String]) -> String {
    ...
    out.push_str(
        "Generated by `crates/jiaochou/tests/projection_register.rs`. **Do not edit by hand** — regenerate with\n\
         `BLESS=1 TIANHENG_WORKSPACE_TESTS=1 cargo test -p jiaochou --test projection_register`.\n\n",
    );
...
fn a_regeneration_command_is_registered_and_never_run() {
    ...
    let tracked_files: BTreeSet<&str> =
        BTreeSet::from(["crates/jiaochou/tests/projection_register.rs"]);
    assert_eq!(
