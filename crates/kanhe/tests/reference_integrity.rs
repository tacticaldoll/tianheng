...
fn package_relative_offence(
    ...
    // precisely because it exists nowhere — a record may note correcting one and name it in the sentence
...
fn offences_in(
    ...
    for rel_path in corpus {
        ...
        for line in content.lines() {
            ...
            // falsification `release-coherence` refuses. An earlier released section may legitimately
            // name paths that genuinely existed at that release and were deleted in subsequent work.
            // Rewriting such entries to satisfy today's tree would falsify the historical record.
            //
            // **It keys on the section and not on the document**, and a by-directory exemption that once
            // stood beside it is retired: the facts a record must keep are shas, dates, versions and counts
            // — **not paths** — so exempting a document for where it lives buys a record nothing it needs
            // and costs every path reference in it.
            //
...
/// A repository whose history records one deletion per extension the bare forms read.
///
/// **`deleted_outside_changes` is read from `git log --diff-filter=D`, and a guard may not take this
/// repository's own history as its subject.** Re-founded at the `0.6.0` snapshot, `main` carries one commit
/// and records no deletion at all, so both bare forms answer over an empty set — and the probes that prove
/// their reach then report clean for a reason unrelated to the reader under test. That is the wrong-grain
/// colour this file's neighbours already name: what produced it was the host's history rather than the form.
///
/// So the deletion belongs to the fixture. It tracks one member, because [`offences_in`] refuses a tree with
/// no member under the crate directory rather than resolving a package-relative path against nothing, and a
/// second commit removes two files — one Rust, one shell, which are the extensions a deletion can carry into
/// the two forms.
///
...
fn a_repository_recording_a_deletion(label: &str) -> (PathBuf, String, String) {
    ...
    let removed = [
        format!("crates/probe/src/{gone_rust}"),
        format!("scripts/{gone_shell}"),
    ];
    for (path, body) in [
        ("crates/probe/Cargo.toml", "[package]\nname = \"probe\"\n"),
        ("crates/probe/src/lib.rs", "pub fn kept() {}\n"),
        (removed[0].as_str(), "pub fn removed() {}\n"),
        (removed[1].as_str(), "#!/usr/bin/env bash\n"),
    ] {
        let full = repo.join(path);
        std::fs::create_dir_all(full.parent().expect("a parent")).expect("a fixture subdirectory");
        std::fs::write(full, body).expect("write a fixture file");
    }
    let git = |args: &[&str]| kanhe::hermetic_git::fixture(&repo, "git", args);
    git(&["init", "-q", "-b", "main"]);
    ...
    git(&[
        ...
        "-q",
        "-m",
        "feat(probe): the tree a later commit removes from",
    ]);
    git(&["rm", "-q", removed[0].as_str(), removed[1].as_str()]);
    git(&["commit", "-q", "-m", "refactor(probe): remove both files"]);
    (repo, gone_rust.to_string(), gone_shell.to_string())
}
...
fn every_extraction_form_is_seen_when_it_names_something_absent() {
    ...
    let planted = [
        ...
        (
            ...
            // top-level directory and the bare form fires only on names a repository once tracked. A
    ...
    // **Two rows are keyed on a DELETION, and the deletion is a fixture's rather than this
    // repository's.** `bare_basename_offence` reports a bare run only when `deleted_outside_changes` holds
    // it — otherwise a bare word is a word, not a reference — so the `zzz`-sentinel form [`planted`]'s rows
    // use leaves both of these inert: measured, a rename to a never-tracked sentinel left one seen by nothing
    // and this direction said so.
    //
    // The premise was owned by whatever this repository had deleted, which made these rows' colour a fact
    // about the host. Re-founded at the `0.6.0` snapshot it has deleted nothing, and both went silent while
    // the forms themselves were unchanged. [`a_repository_recording_a_deletion`] owns the premise now.
    //
    // The names are NOT repeated in this comment, deliberately: `in_repository_references_resolve` reads
    // executed Rust text, and a comment naming a deleted path is a stale reference to it.
    let (history, gone_rust, gone_shell) = a_repository_recording_a_deletion("deletion-history");
    let history_tracked = tracked(&history);
    let planted_by_history = [
        (
            "a bare basename",
            "probe-basename.md",
            format!("The `{gone_shell}` gate says so.\n"),
        ),
        (
            // A bare RUST basename, which the form excluded until the 0.5.0 window: the extension list
            // admitted the governance extensions only, so the branch below was inert for every Rust file a
            // repository could delete. Its own row rather than trust in the `.sh` one, because what was
            ...
            format!("The shadowing lives in `{gone_rust}`.\n"),
        ),
    ];

    for (_, path, body) in planted.iter().chain(planted_by_history.iter()) {
...
///
/// Why the blindness is right where it is: an earlier released section may legitimately name paths
/// that genuinely existed at that release and were deleted when machinery migrated. Rewriting such
/// entries to satisfy today's tree would falsify the historical record — a human falsifying a record
/// to satisfy a check.
///
/// Why it stops there. A by-directory exemption once stood beside this one and is retired: the facts a record
/// must keep are shas, dates, versions and counts, and none of those is a path, so exempting a document for
/// where it lives buys a record nothing it needs and costs every path reference in it.
#[test]
...
/// bare Rust basenames a document names, the ones the repository once tracked and no longer does were the
...
///
/// **Both directions judge a fixture, and the positive one is why.** It needs a repository that has deleted
/// a Rust file, which this one had until it was re-founded at the `0.6.0` snapshot and now has not — so the
/// control went silent while the form was unchanged, and a green here would have been drawn from the host's
/// history. [`a_repository_recording_a_deletion`] supplies the deletion; the negative direction takes the
/// same tree so the pair differs in the name alone, which is the whole claim.
#[test]
fn a_bare_rust_basename_reacts_only_for_a_name_this_repository_deleted() {
    let Some(_) = workspace_root() else {
        return;
    };
    let (history, gone_rust, _) = a_repository_recording_a_deletion("rust-basename-history");
    let history_tracked = tracked(&history);
    let scratch = scratch("rust-basename");

    let deleted = "probe-deleted.md";
    let never = "probe-never-tracked.md";
    std::fs::write(
        scratch.join(deleted),
        format!("The shadowing lived in `{gone_rust}`.\n"),
    )
    ...
    assert!(
        ...
        "a bare Rust basename the repository under test deleted must be refused, or the silence below proves \
         nothing"
    );
    assert!(
        seen_never.is_empty(),
        "a bare Rust basename never tracked is an illustrative name, not a path, but got:\n{}",
...
/// that the record change every time the tree does — `AGENTS.md` names exactly two such carriers, and this
/// is the one that lives inside a file the rest of which is live. A commit message is the other, and it is
/// not a file.
///
...
fn markdown_anchor_offences_in(corpus_root: &Path, corpus: &[String]) -> BTreeSet<String> {
    ...
    for path in corpus.iter() {
        ...
        // heading wherever one appeared — `AGENTS.md` enumerates two record carriers and a level-2 heading
