...
fn a_closed_entry_does_not_stay_under_a_live_class() {
    ...
    let misfiled = misfiled_closed_entries("BACKLOG.md", &text);
    assert!(
        misfiled.is_empty(),
...
/// The mark this reader reads a closed entry by, shown on text this file supplies.
///
/// **A floor over `BACKLOG.md` cannot carry this, because an empty scan there has two causes.** One is the
/// mark having moved, which is blindness; the other is a repository where nothing is closed yet, which is
/// clean. A count asserted over that document reads the second as the first — met at the `0.6.0` snapshot,
/// where the closed-records section holds every struck-through bullet the repository has and the mark had not
/// moved at all. So the reach is proved here and the judgement above reads the repository, which is the split
/// every gate standing in front of an act already uses.
///
/// Both halves on one fixture, because a reader that reports everything passes the first alone: a
/// struck-through bullet under a live class heading is reported, and one under the closed-records heading —
/// which names no class — is not. Perturb `- ~~` to any other mark and both halves go silent together.
#[test]
fn the_closed_entry_mark_is_the_one_this_reader_reads_by() {
    let fixture = "## Live decision index\n\
                   \n\
                   ### READY-PATCH\n\
                   \n\
                   - ~~A closed entry left under a live class~~ — the shape the direction above refuses.\n\
                   - A live entry, carrying no mark.\n\
                   \n\
                   ### Closed — reproduction records\n\
                   \n\
                   - ~~A closed entry where closed entries belong~~ — named by nothing.\n";
    let misfiled = misfiled_closed_entries("fixture.md", fixture);
    assert_eq!(
        misfiled.len(),
        1,
        "the bullet under `READY-PATCH` is reported and the one under the closed-records heading is not: {}",
        misfiled.join("\n")
    );
    assert!(
        misfiled[0].contains("READY-PATCH"),
        "the report names the live heading the entry sits under: {}",
        misfiled[0]
    );
}

/// Every struck-through bullet of `text` that sits under a heading naming a live class, labelled `label`.
///
/// The mark is a literal and whether the heading above it names a class is the question
/// [`heading_classes`] already answers, so both halves are decidable. `label` is a parameter because the
/// diagnostic names the document it read, and a fixture must not be reported as one it did not.
fn misfiled_closed_entries(label: &str, text: &str) -> Vec<String> {
