...
///
/// **The list is hand-written, and each entry is a measurement made before the write.** `xuanji` is
/// here because its `//` count outside `tests.rs` was measured at zero before the entry landed;
/// every later entry earns its place the same way — the crate's `//` population moved first, then
/// its name was written here. The per-crate guard in the gate below refuses any listed name that
/// contributes no file, so a typo or an unpublished crate cannot enter silently.
