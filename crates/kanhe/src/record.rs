...
//!
//! `AGENTS.md` names the records: a commit message and a dated `CHANGELOG.md` section. A record
...
//! here.
//!
//! A commit message is not a tracked file and needs no row, so the one that is a file is the whole of this
//! module: adding a document to escape a refusal is the move it exists to make visible.
//!
//! **A by-prefix half was retired with its only instance.** One directory was a record in whole, and the
//! document it held restated two properties `AGENTS.md` carries itself — that `cargo publish` records the
//! commit rather than the content, and that a published snapshot amended afterwards orphans a pointer the
//! gate cannot foresee. With the directory retired the prefix matched nothing, and a classifier with no
//! instance is a name without a reaction. The path is not written here for the reason
//! `in_repository_references_resolve` gives: a comment naming a retired directory is a stale reference to
//! it.
...
///
/// Empty for live text, and the dated sections alone for the sectioned one.
