...
pub fn observation_bounds() -> Vec<BoundDecl> {
    vec![
        ...
        // at all until this one, while carrying a blanket exemption by directory that no specification
        ...
        // dated sections, and false of `BACKLOG.md`, `AGENTS.md` and the specifications,
        ...
        BoundDecl::pinned(
            ...
            // is wrong. Something was, and the classification was settled on it: a released section named
            // machinery an adopter reading it still met, which is exactly the harm this rule exists to stop.
            // That instance left the tree with the history it sat in, so the leak is currently **unreachable
            // rather than absent** — reached again by the first released section this repository carries that
            // names a path since deleted. Saying *may* here would demote a settled classification to a
            // possibility. What is refused is
            ...
            Extent::Reached(Reached::UnderReacts {
                because: "a dated section records what was true at that release, so rewriting it to satisfy a \
                          rule written afterwards would falsify the record rather than repair it. The leak is real and stays: an adopter reading \
                          a released section can meet entries naming files they can never run, and closing it needs a \
