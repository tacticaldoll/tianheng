...
| 渾儀 (semantic) | [`hunyi`](https://crates.io/crates/hunyi) | type exposure, impl locality, visibility, forbidden markers, `dyn` & `impl Trait` (existential) exposure (shape-only & named-operand), `async fn` (implicit existential) exposure (AST/`syn`) |
