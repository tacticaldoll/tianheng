# The projection register

Every generated document in this repository, the reaction that holds it fresh, and the command its own
header names for regenerating it. Enumerated from tracked content by the marker each document carries, so
a document enters this table the moment it declares itself generated.

Generated by `crates/jiaochou/tests/projection_register.rs`. **Do not edit by hand** — regenerate with
`BLESS=1 TIANHENG_WORKSPACE_TESTS=1 cargo test -p jiaochou --test projection_register`.

## What registration in this table does not mean

**Not freshness.** Each document's own holder asserts that its content matches what the code would
produce, and duplicating that here — inside the register built to end duplication — would refute itself.
This table says a document is known, has a reaction behind its warning, and can be found.

**Not that the command works.** The regeneration command in each header is registered and never run.
Verifying it would mean re-entering the `cargo test` harness already running, or letting this reaction
write into the tree it judges. A header naming a command that regenerates nothing is invisible here, and
that is a declared observation bound rather than an oversight.

**Not that the set is complete.** One mechanism is recognized: a Rust call to the shared blessing
rule. A document generated some other way, whose author also omitted the marker, is absent from both
sides of the correspondence — a declared false negative owned by this engine, not a limit of what it
can read.

A second was recognized until this window: a `check_*` gate writing its projection under `BLESS`. No
tracked unit is one, so that arm asserts its own emptiness rather than being pruned — if such a unit
exists again the reaction says so, which is the difference between retiring a recognizer and
forgetting it.

## The register

| document | held fresh by | regenerate with |
| --- | --- | --- |
| `AGENTS.self-law.md` | `crates/shengmo/tests/self_governance.rs` | `BLESS=1 cargo test -p shengmo self_law_projection_is_fresh` |
| `docs/observation-bound-extents.md` | `crates/jiaochou/tests/observation_bound_model.rs` | `BLESS=1 TIANHENG_WORKSPACE_TESTS=1 cargo test -p jiaochou --test observation_bound_model` |
| `docs/observation-bounds.md` | `crates/jiaochou/tests/bound_register.rs` | `BLESS=1 TIANHENG_WORKSPACE_TESTS=1 cargo test -p jiaochou --test bound_register` |
| `docs/projection-register.md` | `crates/jiaochou/tests/projection_register.rs` | `BLESS=1 TIANHENG_WORKSPACE_TESTS=1 cargo test -p jiaochou --test projection_register` |

4 documents.

## The reactions holding them

Enumerated independently of what the documents claim, because a document naming its generator is a claim
by the document and the call site is the fact:

- `crates/jiaochou/tests/bound_register.rs`
- `crates/jiaochou/tests/observation_bound_model.rs`
- `crates/jiaochou/tests/projection_register.rs`
- `crates/shengmo/tests/self_governance.rs`

4 reactions.
